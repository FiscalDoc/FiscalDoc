          ...
          <table class="table">
            <thead>
              <tr>
                ...
                <th>Razão Social</th><th>CNPJ</th><th>Cidade/UF</th><th>Status</th><th></th>
              </tr>
            </thead>
            <tbody>
              @for (t of transportadoras(); track t.id) {
                <tr class="row-link" [class.row-selected]="selecionados().has(t.id)" (click)="abrirTransportadora(t.id)">
                  <td class="checkbox-cell"><input type="checkbox" [checked]="selecionados().has(t.id)" (change)="toggleSelecionado(t.id)" (click)="$event.stopPropagation()"/></td>
                  <td>
...
export class TransportadorasListComponent implements OnInit {
  ...
  readonly todosSelecionados = computed(() => {
    const itens = this.transportadoras();
    return itens.length > 0 && itens.every(t => this.selecionados().has(t.id));
  });
  ...
  exportar(): void {
    ...
    const ativo = this.filtroAtivo();
    this._svc.exportarXlsx(this.clienteId, this.termo || undefined, ativo ?? undefined).subscribe({
      next: blob => { this.exportando.set(false); this._triggerDownload(blob, 'transportadoras.xlsx'); },
      error: err => { this.exportando.set(false); this._toast.error(extractErrorMessage(err, 'Erro ao exportar transportadoras.')); },
    });
  }
  ...
  onImportarArquivo(e: Event): void {
    ...
    this._svc.importarXlsx(this.clienteId, arquivo).subscribe({
      next: r => {
        this.importando.set(false);
        if (r.criados > 0) this._toast.success(`${r.criados} transportadora(s) importada(s)!`);
  ...
  toggleSelecionarTodos(): void {
    ...
    this.selecionados.set(new Set(this.transportadoras().map(t => t.id)));
  }
  ...
  bulkAtivar(ativo: boolean): void {
    ...
    this._svc.bulkAtivar(this.clienteId, ids, ativo).subscribe({
      next: () => {
        this.processandoLote.set(false);
        this._toast.success(`${ids.length} transportadora(s) ${ativo ? 'ativada(s)' : 'desativada(s)'}!`);
        this.buscar();
      },
      error: err => { this.processandoLote.set(false); this._toast.error(extractErrorMessage(err, 'Erro ao atualizar transportadoras.')); },
    });
  }
  ...
  async bulkExcluir(): Promise<void> {
    if (this.processandoLote()) return;
    const ids = Array.from(this.selecionados());
    const ok = await this._confirm.ask(`Excluir ${ids.length} transportadora(s) selecionada(s)? Esta ação não pode ser desfeita.`, { confirmLabel: 'Excluir' });
    if (!ok) return;

    this.processandoLote.set(true);
    forkJoin(ids.map(id => this._svc.delete(this.clienteId, id).pipe(catchError(() => of('erro' as const))))).subscribe(resultados => {
      this.processandoLote.set(false);
      const falhas = resultados.filter(r => r === 'erro').length;
      const sucesso = resultados.length - falhas;
      if (sucesso > 0) this._toast.success(`${sucesso} transportadora(s) excluída(s)!`);
      if (falhas > 0) this._toast.error(`${falhas} transportadora(s) não puderam ser excluídas.`);
      this.buscar();
    });
  }
