            ...
            <div class="title-row">
              ...
              @if (!isNew() && vizinhoTotal()) {
                <div class="nav-vizinhos">
                  <button type="button" class="nav-btn" [disabled]="!vizinhoAnteriorId()" (click)="irParaAnterior()" [title]="vizinhoAnteriorLabel() || 'Sem transportadora anterior'">
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
                    </svg>
                    Anterior
                  ...
                  <button type="button" class="nav-btn" [disabled]="!vizinhoProximoId()" (click)="irParaProximo()" [title]="vizinhoProximoLabel() || 'Sem próxima transportadora'">
                    Próximo
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
export class TransportadoraDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // pra outro :transportadoraId dentro da MESMA rota — o Angular reaproveita esta instância
    // do componente nesse caso, então um ngOnInit que só rodasse uma vez nunca pegaria a troca.
    this._route.paramMap.subscribe(params => {
      this.clienteId = params.get('id')!;
      this.transportadoraId = params.get('transportadoraId') ?? '';
  ...
  private _carregarVizinhos(): void {
    this._svc.getVizinhos(this.clienteId, this.transportadoraId).subscribe({
      next: v => {
  ...
  irParaAnterior(): void {
    ...
    if (id) this._router.navigate(['/clientes', this.clienteId, 'cadastros', 'transportadoras', id]);
  }
