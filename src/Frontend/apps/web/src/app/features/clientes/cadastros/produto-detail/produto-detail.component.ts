...
export class ProdutoDetailComponent implements OnInit {
  ...
  private _carregarVizinhos(): void {
    this._svc.getVizinhos(this.clienteId, this.produtoId).subscribe({
      next: v => {
  ...
  irParaAnterior(): void {
    ...
    if (id) this._router.navigate(['/clientes', this.clienteId, 'cadastros', 'produtos', id]);
  }
  ...
  async excluir(): Promise<void> {
    ...
    this._svc.delete(this.clienteId, this.produtoId).subscribe({
      next: () => { this._toast.success('Produto excluído!'); this.goBack(); },
      error: err => {
        const msg = extractErrorMessage(err, 'Erro ao excluir produto.');
        this.erro.set(msg);
